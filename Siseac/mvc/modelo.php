...
function SelecionarTodosDocentes($idGrupo,$idperiodo) {
	...
	//echo "<select id='tutor' name='tutor[]' data-placeholder='Selecciona un docente...' class='validate[required] chzn-select' tabindex='2' >";
...
function VerificaDesgloseCargaTutores ($id_docente, $id_concepto, $id_carrera,$grupot, $actualizacion, $horas_asignadas,$descripcion) {
	$filaPeriodo = $this->regresaPeriodoActual();
	$id_periodo = $filaPeriodo[0];
...
function VerificaDesgloseCargaLaboratorios ($id_docente, $id_concepto, $id_carrera,$laboratoriot, $actualizacion, $horas_asignadas,$descripcion) {
	$filaPeriodo = $this->regresaPeriodoActual();
	$id_periodo = $filaPeriodo[0];
...
function VerificaCargaIndividual ($id_docente,$id_concepto,$id_carrera,$actualizacion,$horas_asignadas,$descripcion) {
	$filaPeriodo = $this->regresaPeriodoActual();
	$id_periodo = $filaPeriodo[0];
...
function VerificaMateCarga($idDocente,$id_materia,$actualizacion,$id_carrera,$id_grupo) {
					$filaPeriodo = $this->regresaPeriodoActual();
					$id_periodo = $filaPeriodo[0];
...
function ObtenerSumatoria($id_carrera){
		$filaPeriodo = $this->regresaPeriodoActual();
		$id_periodo = $filaPeriodo[0];
...
function obtieneIdProyecto($id_carrera,$id_docente,$id_concepto){
	$filaPeriodo = $this->regresaPeriodoActual();
	$id_periodo=$filaPeriodo[0];
...
function VerificaExisteConcepto($id_docente, $id_concepto, $id_carrera){
	$filaPeriodo = $this->regresaPeriodoActual();
				$id_periodo=$filaPeriodo[0];
...
function ptcVSpaporCarrera($carr,$abrevia,$periodo){
	if($periodo==''){
		$filaPeriodo = $this->regresaPeriodoActual();
		$id_periodo = $filaPeriodo[0];		
	}else{
	...
	function regresaPeriodoActual(){
		$sql = "SELECT idperiodo_escolar as id, concat(periodo,' ',anual) as periodo FROM periodo_escolar WHERE activo = 'si' ";
		$result = mysql_query($sql);
		$fila = mysql_fetch_row($result);
		return $fila;
	}
	...
	function listaPeriodos(){
		$sql = "SELECT idperiodo_escolar as id, concat(periodo,' ',anual) as periodo, activo FROM periodo_escolar";
	...
	function atualizaPeriodoActual($id){
		$sql = "UPDATE periodo_escolar SET activo = 'no'";
		$result = mysql_query($sql);
		$sql = "UPDATE periodo_escolar SET activo = 'si' WHERE idperiodo_escolar = '$id'";
		$result = mysql_query($sql);
	}

	function eliminaGrupo($idGrupo){
		//$sql = "SELECT count(idtutores_grupo) FROM tutores_grupo WHERE id_grupo='$idGrupo'";
		//$result = mysql_query($sql);
		//$fila = mysql_fetch_row($result);
		//if($fila[0]==0){
		...
		//}else{
		//	$mensaje = "no se puede eliminar el grupo";
		//}

			if(!$result){
				$mensaje = "El grupo ya contiene Datos Relacionados";
			}else{
				$mensaje = "grupo eliminado";
			}
		return $mensaje;

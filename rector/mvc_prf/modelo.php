...
function SelecionarDocentesCargadoTutores($idgrupo) {
			$idgrupo;
				$sql = "SELECT prf_datopersonales.id, nombre,ap_paterno,ap_materno FROM tutores_grupo,prf_datopersonales WHERE prf_datopersonales.id=tutores_grupo.id_docente and  tutores_grupo.id_grupo='$idgrupo';";
				$result = mysql_query($sql);
				$i = 0;
...
function SelecionarConceptoCargaHoraria($tipo) {
				$sql = "SELECT idconceptos_carga_horaria,nombre FROM conceptos_carga_horaria where tipo ='$tipo';";
				$result = mysql_query($sql);
				$i = 0;
							echo"  <select   name='conceptosCH' data-placeholder='Selecciona un docente...'  chzn-select' tabindex='2' >";
							echo"  <option value=''></option> ";
...
function VerificaDesgloseCargaTutores ($id_periodo,$id_docente, $id_concepto, $id_carrera,$grupot, $actualizacion, $horas_asignadas,$descripcion) {
				$sql = "SELECT * FROM desglose_carga_horaria where grupo='$grupot'and id_concepto=6 and id_periodo='$id_periodo';";
				$result = mysql_query($sql);
				$res = mysql_num_rows($result);
...
function UpdaDesgloseCargaTutores($idDocente,$actualizacion,$idgrupo) {
...
function VerificaDesgloseCargas ($id_periodo, $id_docente, $id_concepto, $id_carrera,$materiaDc,$grupoDc, $actualizacion, $horas_asignadas,$descripcion) {
			...
			if ($res > 0) {
						//echo "Actualizar desglose";
					$ActualizarTutores = $this->UpdaDesgloseCarga($id_docente,$actualizacion,$grupoDc,$materiaDc);
					  } 
			else {
						//echo "Insertar desglose Horaria";
...
function UpdaDesgloseCarga($idDocente,$actualizacion,$grupo,$materia) {
					$result = mysql_query("UPDATE desglose_carga_horaria SET  id_docente='$idDocente', actualizacion='$actualizacion' WHERE grupo='$grupo' and materia='$materia';");
			if (!$result) {
					$error = 'Invalid query: ' . mysql_error();
						echo '<script type="text/javascript">';
...
function VerificaCargaIndividual ($id_periodo, $id_docente,$id_concepto,$id_carrera,$actualizacion,$horas_asignadas,$descripcion) {
				$sql = "SELECT *  FROM desglose_carga_horaria,conceptos_carga_horaria 
						...
						AND conceptos_carga_horaria.tipo=2 and conceptos_carga_horaria.idconceptos_carga_horaria='$id_concepto'and desglose_carga_horaria.id_docente='$id_docente' and id_periodo='$id_periodo';";
				$result = mysql_query($sql);
				$res = mysql_num_rows($result);
...
function UpdaCargaIndividual($idDocente,$horasAsignadas,$descripcion,$actualizacion,$idConcepto,$id_periodo) {
				$result = mysql_query("UPDATE desglose_carga_horaria SET  id_docente='$idDocente', horas_asignadas='$horasAsignadas',descripcion='$descripcion',actualizacion='$actualizacion' WHERE id_docente='$idDocente' and id_concepto='$idConcepto' and id_periodo='$id_periodo';");
        if (!$result) {
					$error = 'Invalid query: ' . mysql_error();
							echo '<script type="text/javascript">';
							echo 'alert("Ocurrio un error intenta nuevamente' . $error . '");';
...
function MateriasPorDoceCH($id_docente) {
				...
				AND  materias_carga_horaria.id_carrera=carreras.id and id_periodo='4';";
				$result = mysql_query($sql);
				$i = 0;
...
function MatePorDoceCHCarera($id_docente,$id_carrera) {
				$sql = "SELECT plan_estudios.nombre_materia, grupo.grado,grupo.grupo,grupo.turno, carreras.abreviatura, plan_estudios.horas_por_semana 
						FROM materias_carga_horaria,plan_estudios,grupo,carreras 
						...
						AND  materias_carga_horaria.id_carrera='$id_carrera' and id_periodo='4';";
				$result = mysql_query($sql);
				$i = 0;
						echo ' <h3>MATERIAS QUE IMPARTE</h3>';
						echo"  <table class='table table-bordered table-striped  data_table3' >
...
function SumarMateriasCH($id_docente) {
				$sql = "select  sum(horas_por_semana) from materias_carga_horaria,plan_estudios where  id_materia=plan_estudios.id  and id_docente='$id_docente' and id_periodo='4';";
				$result = mysql_query($sql);
				$i = 0;
...
function SumarMateriasCHPorCarrere($id_docente,$carrera) {
				$sql = "select  sum(horas_por_semana) from materias_carga_horaria,plan_estudios where  id_materia=plan_estudios.id  and id_docente='$id_docente' and id_carrera='$carrera' and id_periodo='4';";
				$result = mysql_query($sql);
				$i = 0;
...
function ObtenerDatosCargahoraria() {
				$sql = "SELECT id_docente,prf_datopersonales.n_empleado , prf_datopersonales.nombre, prf_datopersonales.ap_paterno,prf_datopersonales.ap_materno ,
						...
						WHERE desglose_carga_horaria.id_docente=prf_datopersonales.id 
							AND desglose_carga_horaria.id_periodo=4
						GROUP BY id_docente ;";
				$result = mysql_query($sql);
				$i = 0;
        while ($fila = mysql_fetch_row($result)) {
        ...
        if ($grado[$i] == 1) {
				...
				$SumaMaterias = $this->SumarMateriasCH($idPrf[$i]);
						...
						echo" </td>";
                        echo" <td>";
						echo $cacademicos[$i] = $fila[11];
						echo" </td>";
                        echo" <td>";
						echo $racademicos[$i] = $fila[12];
						echo" </td>"; 
						echo" <td>";
						echo $tlab[$i] = $fila[13];
						echo" </td>"; 
						echo" <td>";
						echo $investigacion[$i] = $fila[14];
						echo" </td>";
						echo" <td>";
						echo $isos[$i] = $fila[15];
						echo" </td>";
                        echo" <td>";
						echo $actualizacion[$i] = $fila[16];
						echo" </td>";
                        echo" <td>";
						echo $difusion[$i] = $fila[17];
						echo" </td>";
						echo" <td>";
						echo'    <p><a href="javascript: void(0)" onclick="window.open( ';
...
function TutoPorGrupo($id_docente) {
				$sql = "SELECT grupo.grado,grupo.grupo, carreras.abreviatura 
						FROM tutores_grupo,grupo,carreras
						WHERE id_docente='$id_docente' 
						AND tutores_grupo.id_grupo=grupo.idGrupo AND grupo.carrera=carreras.id;";
				$result = mysql_query($sql);
				$i = 0;
...
function ObtenerDocentePorMateriaI() {
...
function ObtenerDatosCargahorariaPorCarrera($carrera) {
					$sql = "SELECT id_docente,prf_datopersonales.n_empleado , prf_datopersonales.nombre, prf_datopersonales.ap_paterno,prf_datopersonales.ap_materno ,
									SUM(CASE WHEN id_concepto = 1 THEN horas_asignadas ELSE 0 END) hsemagrupo,
									SUM(CASE WHEN id_concepto = 2 THEN horas_asignadas ELSE 0 END)  frentegrupo,
									SUM(CASE WHEN id_concepto = 3 THEN horas_asignadas ELSE 0 END) asesorias,
        ...
        if ($grado[$i] == 1) {
					...
					$frentegrupo[$i] = $fila[6];
								...
								echo ' return false;">';
									//echo $frentegrupo[$i];
					$SumaMaterias = $this->SumarMateriasCHPorCarrere($idPrf[$i],$carrera);
								echo $SumaMaterias;
								echo '</a></p>'; 
								echo" </td>";
								echo" <td>";
        ...
        if($sumaTotal<=40)
								...
								echo         $sumaTotal;
								echo" </td>";
                    $TotalInstitucional = $this->TotalInstitucional($idPrf[$i]);
 ...
 function TotalInstitucional($idDocente) {
					$sql = "SELECT id_docente,prf_datopersonales.n_empleado , prf_datopersonales.nombre, prf_datopersonales.ap_paterno,prf_datopersonales.ap_materno ,
									SUM(CASE WHEN id_concepto = 1 THEN horas_asignadas ELSE 0 END) hsemagrupo,
									SUM(CASE WHEN id_concepto = 2 THEN horas_asignadas ELSE 0 END)  frentegrupo,
									SUM(CASE WHEN id_concepto = 3 THEN horas_asignadas ELSE 0 END) asesorias,
...
function CarreraPorDirectorSelect($idDirector) {
					...
					$i = 0;
								...
								echo '  <div>';
								echo"   <select   name='id_carrera' data-placeholder='Selecciona una carrera...'  chzn-select' tabindex='2' >";
								echo"   <option value=''></option> ";
        while ($fila = mysql_fetch_row($result)) {
            		$id_carrera[$i] = $fila['0'];                
								echo"  <option value='"; 
...
function ReporteporDirector($idDirector) {
					$sql = "SELECT carreras.id, carreras.nombre, carreras.nivel,carreras.abreviatura  
							FROM director_carrera, 01_user,carreras
							WHERE  01_user.id=director_carrera.user_ide and 01_user.id='$idDirector' and carreras.id=director_carrera.carrera;";
					$result = mysql_query($sql);
        ...
        while ($fila = mysql_fetch_row($result)) {
					...
					$idCarrer[$i] = $fila[0];
					$Cargahoraria=$this->ObtenerDatosCargahorariaPorCarrera($idCarrer[$i]);
									//           echo" <td>";
									//           $idCarrer[$i] = $fila[0];
									//           $abreviatura[$i] = $fila[3];
									//           echo"<a href='CargarLaboratorios.php?idCarre=$idCarrer[$i]&abrevi=$abreviatura[$i]'>";
